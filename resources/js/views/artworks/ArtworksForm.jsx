...
const ArtworksForm = () => {
  ...
  const loadArtists = async () => {
    try {
      ...
      const artistsData = response.data.artists || response.data.data || response.data || [];
      setArtists(Array.isArray(artistsData) ? artistsData : []);
    } catch (err) {
      console.error('Error loading artists:', err);
      setArtists([]); // Ensure it's always an array
    }
  };
  ...
  const loadTags = async () => {
    try {
      ...
      setAvailableTags([]); // Ensure it's always an array
    }
  };
  ...
  const loadArtwork = async () => {
    ...
    try {
      ...
      const artwork = response.data.work || response.data.artwork || response.data;
      
      setFormData({
        title: artwork.title || '',
        description: artwork.description || artwork.overview || '',
      ...
      if ((artwork.images || artwork.image_urls) && (artwork.images || artwork.image_urls).length > 0) {
        const images = artwork.images || artwork.image_urls;
  ...
  const handleTagsChange = (event, newValue) => {
    setFormData(prev => ({ ...prev, tags: Array.isArray(newValue) ? newValue : [] }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        // Always set remove_cover_image field for updates
        submitData.remove_cover_image = !existingImages.cover;
        
        // Handle removed gallery images - send array of remaining image IDs
        if (existingImages.gallery && existingImages.gallery.length > 0) {
          submitData.existing_image_ids = existingImages.gallery.map(img => img.id);
        } else {
          submitData.existing_image_ids = [];
        }
      }
    ...
    <MainCard title={isEdit ? 'Edit Artwork' : 'Create Artwork'}>
      ...
      <Box component="form" onSubmit={handleSubmit} noValidate>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <Autocomplete
            fullWidth
            options={Array.isArray(artists) ? artists : []}
            getOptionLabel={(option) => option.artist_name || option.name || `Artist ${option.id}`}
            value={artists.find(artist => artist.id === formData.artist_id) || null}
            onChange={(event, newValue) => {
              setFormData(prev => ({
                ...prev,
                artist_id: newValue ? newValue.id : ''
              }));
            }}
            disabled={loading}
            renderInput={(params) => (
              <TextField
                ...
                label="Artist"
                required
                helperText={`${artists.length} artists loaded`}
              ...
              <Box component="li" {...props}>
                <Box>
                  <Typography variant="body1">
                    {option.artist_name || option.name || `Artist ${option.id}`}
                  </Typography>
                  {option.specialties && (
