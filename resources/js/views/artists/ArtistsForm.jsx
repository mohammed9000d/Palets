...
const ArtistsForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        // Always set remove_avatar field for updates
        submitData.remove_avatar = !existingImages.avatar;
        
        // Handle removed gallery images - send array of remaining image IDs
        if (existingImages.gallery && existingImages.gallery.length > 0) {
          submitData.existing_gallery_ids = existingImages.gallery.map(img => img.id);
        } else {
          submitData.existing_gallery_ids = [];
        }
      }
